...
Our dataset includes random frames of 2 versus 2 player Rocket League games that includes location data of all 4 players, car features, ball position, and whether or not a particular shot resulted in a goal. The dataset is from [ website -- to write ]
...
However, there were a number of significant variables, including: the $y$ and $z$ coordinates of the ball position, the ball velocity, the shot taker location, the opponent's velocity, the distance between the shot taker and the two opponents, and the angle between the shot taker and the two opponents. While the coordinates of the ball position, the shot taker's location, and the distance between the shot taker and the two opponents would influence goal probability as expected, it is interesting that ball velocity and the angle between the shot taker and the two opponents also impact xG. The coefficient for the ball velocity in the $y$ and $z$ directions are positive, which implies that shot takers who strike the ball when it's moving faster are more likely to score a goal. Moreover, the coefficient for the angle between both opponents is positive, which implies [ linda ]
...
3. from the correlation plot, outperformance is correlated with actual goal rate which is expected if the model is robust, actual goal rate is correlated with avg xg, suggesting that a lot of the ability to score goals comes from positioning/ball stuff (instead of randomness).

- outperformance not correlated with avg xg? outperformance = total actual goals / sum of xg
